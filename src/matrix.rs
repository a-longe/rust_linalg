...
impl<T: VectorItem, const R:usize, const C:usize> Matrix<T, R, C> {
    ...
    pub fn get_row(&self, r_i: usize) -> Option<Vector<T, C>> {
        if r_i >= R { return None; }
        let mut row_vec: Vector<T, C> = Vector::new();
        for c_i in 0..C {
            row_vec.set(c_i, self.get(r_i, c_i).expect("Matrix is larger than type suggests"));
        }
        Some(row_vec)
    }
    ...
    pub fn get_cols<'a>(&'a self) -> Iter<'a, Vector<T, R>> {
        self.items.iter()
    }
    ...
    pub fn get_transpose(&self) -> Matrix<T, C, R> {
        ...
        for r_i in 0..R {
            for c_i in 0..C {
                transpose.set(c_i, r_i, self.get(r_i, c_i).expect("Matrix is larger than type suggests"));
            }
        }
        transpose
    }
    pub fn get_rows<'a>(&'a self) -> Iter<'a, Vector<T, C>> {
        unimplemented!();
    }
}
...
impl<T: VectorItem, const R:usize, const S:usize, const C:usize> Mul<Matrix<T, S, C>> for Matrix<T, R, S> {
    type Output = Matrix<T, R, C>;
...
impl<T: VectorItem, const R:usize, const C:usize> Iterator for Matrix<T, R, C> {
    type Item = Vector<T, R>;
    fn next(&mut self) -> Option<Self::Item> {
        self.get_cols().next().copied()
    }
}
